 ...
 * @author jade
 */
public class PuntosFigu implements Iterable<PuntoFig>, IDrawable {

	private PuntoFig[] puntos;

	public PuntosFigu() {
		this(0);
	}

	public PuntosFigu(int capacity) {
		puntos = new PuntoFig[capacity];
	}

	public boolean setValueAt(int index, PuntoFig point) {
	...
	public boolean setValueAt(int index, Point point) {
		...
		try {
			puntos[index] = new PuntoFig((double) point.x, (double) point.y);
		} catch (Exception ex) {
			return false;
		}
	...
	public PuntoFig getValueAt(int index) {
	...
	public PuntoFig[] sort() {

		PuntoFig[] aux = puntos.clone();
